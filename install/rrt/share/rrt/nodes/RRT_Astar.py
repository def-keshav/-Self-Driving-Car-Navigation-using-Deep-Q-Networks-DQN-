...
#     plt.show()
#     surface = plt.figure()
#     surface = surface.add_subplot(111)
#     surface.set_title('RRT-Astar')
#     surface.set_xlim(0, 500)
#     surface.set_ylim(0, 500)
#     white = 255, 255, 255
#     black = 20, 20, 40
#     surface.fill(white)
#     environment(surface)
...
#           surface.plot([nn.x, newnode.x], [nn.y, newnode.y], color="blue")
#           node_list = rewire_tree(node_list, newnode, surface)
#           plt.draw()
        
#           for e in plt.event.get():
#             if e.type == QUIT or (e.type == KEYUP and e.key == K_ESCAPE):
#                 sys.exit("Leaving because you requested it.")
#     draw_tree(start, goal, node_list, surface)
#     plt.draw()
...
# def rewire_tree(node_list, newnode, surface):
#     white = 255, 240, 200
#     black = 0, 0, 0
...
#             surface.plot([p.x, p.parent.x], [p.y, p.parent.y], color="white")
#             p.parent = newnode
#             p.cost = newnode.cost + euclid_dist([p.x, p.y], [newnode.x, newnode.y]) 
#             node_list[i] = p  
#             surface.plot([p.x, newnode.x], [p.y, newnode.y], color="black")                    
#     return node_list
...
#     height = 10
#     robot_radius  = 0.038
#     clearance = float(input("Enter obstacle clearance for robot: "))

#     RPM1, RPM2 = map(int, input("Enter left wheel and right wheel RPMs: ").split())

#     s_x, s_y = map(int, input("Enter start coordinates: ").split())
#     g_x, g_y = map(int, input("Enter goal coordinates: ").split())

#     if not Valid_move(s_x, s_y, robot_radius, clearance):
#         print("Invalid start node or in obstacle space")
#         exit(-1)
        
#     if not Valid_move(g_x, g_y, robot_radius, clearance):
#         print("Invalid goal node or in obstacle space")
#         exit(-1)
    
#     s_t = int(input("Enter orientation of the robot at start node (must be multiple of 30): "))
    
#     if not validorient(s_t):
#         print("Orientation must be a multiple of 30")
#         exit(-1)
    
#     g_t = int(input("Enter orientation of the robot at goal node (must be multiple of 30): "))
...
#     start_node = Node(s_x, s_y, -1, s_t, 0, 0, 0, 0, 0, 0)
#     goal_node = Node(g_x, g_y, -1, g_t, 0, 0, 0, 0, 0, 0)
    
#     result, Node_List, Path_List = RRT_Astar(start_node, goal_node, RPM1, RPM2, robot_radius, clearance)
    
#     if result == 1:
#         print("Path Found")
#         x, y, theta, ul, ur = Backtrack(goal_node)
#         plt.plot(x, y, 'b-')
#         plt.grid(True)
#         plt.pause(0.01)
#         plt.show()
